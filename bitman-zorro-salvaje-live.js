...
// ------------------------------------------------------------
// Hermano de "El Cauto Temerario", con el mismo motor de señal
// (4H confirmado por Diario, sin SL), pero con un reparto del TP
// parcial más arriesgado: cierra solo el 20% al llegar al TP,
// deja correr el 80% restante sin ninguna protección. Validado
// el 2 de agosto de 2026 (BTC 10/10 años positivos, ETH 9/10)
// como el mejor punto de drawdown con el 5x real — ver la
// Sección -5 de ESTRATEGIA-confluencia-sin-sl.md.
...
// (guardado en state-zorro-salvaje.json), porque el aviso de salida
...
const FRACCION_TP_PARCIAL = 0.20; // al tocar el TP, cierra el 20% y deja correr el 80% (validado como el mejor punto de drawdown con 5x)
const MODO_PRUEBA = true; // deja este aviso en cada mensaje mientras se observa el comportamiento en vivo

const STATE_FILE = path.join(__dirname, 'state-zorro-salvaje.json');
...
// Implementa la configuración FINAL validada: al tocar el TP, cierra el 20%
...
async function main(){
  const now = new Date();
  console.log('El Zorro Salvaje — comprobando ' + SYMBOL + ' a las ' + now.toISOString());
  ...
  if(messages.length){
    const header = '🐺 <b>El Zorro Salvaje · ' + SYMBOL + '</b>\n\n';
