...
// (guardado en state-cauto-temerario.json), porque el aviso de salida
...
const STATE_FILE = path.join(__dirname, 'state-cauto-temerario.json');
...
async function main(){
  const now = new Date();
  console.log('El Cauto Temerario — comprobando ' + SYMBOL + ' a las ' + now.toISOString());
  ...
  if(messages.length){
    const header = '🦊 <b>El Cauto Temerario · ' + SYMBOL + '</b>\n\n';
